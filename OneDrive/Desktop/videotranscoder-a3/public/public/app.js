const BASE_URL_auth='http://localhost:3001'; //change URL accordingly
const BASE_URL_video='http://localhost:3002'; //change URL accordingly
...
function login() {
    ...
    fetch(`${BASE_URL_auth}/auth/login`, {
        method: 'POST',
...
async function loadFiles() {
    try {
        const response = await fetch(`${BASE_URL_video}/files`, {
            method: 'GET',
...
async function deleteFile(fileId) {
    ...
    try {
        const response = await fetch(`${BASE_URL_video}/files/${fileId}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        });
...
async function uploadVideo() {
    ...
    try {
        const response = await fetch(`${BASE_URL_video}/files/upload`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
            body: formData
        });
...
async function downloadFile(fileId, filename) {
    try {
        ...
        const response = await fetch(`${BASE_URL_video}/files/download/${fileId}`, {
            method: 'GET',
            headers: {
...
async function startTranscoding() {
    ...
    try {
        ...
        const ws = new WebSocket(`ws://${BASE_URL_video}`); //change to REAL DNS

        ws.onopen = async () => {
            console.log('WebSocket connection established');

            // Request to initiate transcoding once WebSocket is ready
            const response = await fetch(`${BASE_URL_video}/videos/transcode/${videoId}`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
            });
